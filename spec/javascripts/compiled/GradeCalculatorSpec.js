(function() {
  ...
  define(['compiled/grade_calculator', 'underscore'], function(GradeCalculator, _) {
    ...
    test("never drop", function() {
      ...
      this.assignment_id = 0;
      this.submissions = [];
      this.setup_grades(this.group, [[100, 100], [42, 91], [14, 55], [3, 38], [null, 1000]]);
      this.group.rules = {
        drop_lowest: 1,
        never_drop: ["3"]
      };
      this.group.assignments.map(function(a) {
        return a.id = a.id.toString();
      });
      this.submissions.map(function(s) {
        return s.assignment_id = s.assignment_id.toString();
      });
      result = GradeCalculator.calculate(this.submissions, [this.group]);
      assertGrade(result, 'current', 145, 229);
      assertDropped(result.group_sums[0].current.submissions, [14, 55]);
      assertGrade(result, 'final', 159, 284);
      return assertDropped(result.group_sums[0]['final'].submissions, [0, 1000]);
    });
