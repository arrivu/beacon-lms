(function() {
  define(['jquery', 'Backbone', 'compiled/views/DialogFormView', 'helpers/assertions', 'helpers/util', 'helpers/jquery.simulate'], function($, Backbone, DialogFormView, assert, util) {
    var assertDialogTitle, closeSpy, model, openDialog, sendResponse, server, trigger, view;
    ...
    module('DialogFormView', {
      setup: function() {
        closeSpy = sinon.spy(DialogFormView.prototype, 'close');
    ...
    return test('closing the dialog calls view#close', function() {
      openDialog();
      util.closeDialog();
      return ok(closeSpy.called);
    });
  });
