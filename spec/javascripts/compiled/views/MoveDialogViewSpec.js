(function() {
  ...
  define(['underscore', 'jquery', 'Backbone', 'compiled/views/MoveDialogView', 'helpers/jquery.simulate'], function(_, $, Backbone, MoveDialogView) {
    ...
    test('parentKey value on the model is updated on save success when the model moves collections', function() {
    ...
    return test('parentKey value on the model is updated on save success and is a string', function() {
      var firstSelect, saveURL;
      ...
      ok(this.model.has('assignment_group_id'));
      return ok(typeof this.model.get('assignment_group_id') === 'string');
    });
  });
