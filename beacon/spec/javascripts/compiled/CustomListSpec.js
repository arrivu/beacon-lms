(function() {
  define(['jquery', 'compiled/widget/CustomList', 'helpers/jquery.simulate'], function($, CustomList) {
    module('CustomList', {
      setup: function() {
        var index, items, _i;
    ...
    return test('should cancel pending remove request on add', function() {
      var el, item;

      el = jQuery(this.lis[0]);
      item = this.list.pinned.findBy('id', 0);
      this.list.remove(item, el);
      this.clock.tick(300);
      ok(this.list.requests.remove[0], 'create a "remove" request');
      this.list.add(0, el);
      this.clock.tick(300);
      return equal(this.list.requests.remove[0], void 0, 'delete "remove" request');
    });
  });
